...
def build_provenance_lines(seed_counts: Dict[str, int],
                           selected_counts: Dict[str, int]) -> List[str]:
    """为生成产物追加最小但有用的 provenance 信息。"""
    lines = [
        f"# Seeds: IPv4={seed_counts['v4']}, IPv6={seed_counts['v6']}",
        f"# Selected: IPv4={selected_counts['v4']}, IPv6={selected_counts['v6']}",
    ]
    ...
    if runner_name:
        lines.append(f"# Verified On: {runner_name}")
...
def generate_hosts_content(entries: List[Tuple[str, str]], ip_type: str,
                           provenance_lines: Optional[List[str]] = None) -> str:
    """生成 hosts 文件内容 (去重)"""
    lines = [
...
def main():
    ...
    seed_counts = {"v4": len(ipv4_ips), "v6": 0}
    ...
    selected_counts = {
        "v4": len(all_results["v4"]),
        "v6": len(all_results["v6"]),
    }
    ...
    if all_results['v4']:
        ...
        content_v4 = generate_hosts_content(
            entries_v4,
            "IPv4 Only",
            build_provenance_lines(seed_counts, selected_counts),
        )
    ...
    if all_results['v6']:
        ...
        content_v6 = generate_hosts_content(
            entries_v6,
            "IPv6 Only",
            build_provenance_lines(seed_counts, selected_counts),
        )
    ...
    if entries_dual:
        ...
        content_dual = generate_hosts_content(
            entries_dual,
            desc,
            build_provenance_lines(seed_counts, selected_counts),
        )
